...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    for (auto it1=boundingBoxes.begin(); it1 != boundingBoxes.end(); ++it1)
    {
        ...
        cout << left << " " << top << " " << right << " " << bottom << " " << it1->lidarPoints.size() << endl;
        if (num_pts  > 1)
        {
            ...
            //putText(topViewImg, str1, cv::Point2f(left-240, bottom+50), cv::FONT_ITALIC, 2, currColor);
            sprintf(str2, "xmin=%2.2f m, yw=%2.2fm", xwmin, ywmax-ywmin);
...
int main()
{
    ...
    //cv::Size  worldSize (10, 25);
    //cv::Size  imageSize (1000, 2000);
    //show3DObjects(boundingBoxes, worldSize, imageSize, true);
    show3DObjects(boundingBoxes, cv::Size(10.0, 25.0), cv::Size(100, 200), true);
